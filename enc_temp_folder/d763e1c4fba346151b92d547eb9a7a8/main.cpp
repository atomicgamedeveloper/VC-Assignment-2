    ...
    while (!glfwWindowShouldClose(window)) {
        auto toBeTimed = [&]() {
            ...
            if (shaderRendering) {
                ...
                float normalizedTransX = (frameTranslationX / (BASE_WIDTH * videoAspectRatio)) * 2.0f * videoAspectRatio;
                float normalizedTransY = (frameTranslationY / BASE_WIDTH) * 2.0f;

                model = glm::translate(model, glm::vec3(normalizedTransX, -normalizedTransY, 0.0f));
                ...
                // SET THE MODEL MATRIX ON THE QUAD OBJECT
                myQuad->addTransform(model);  // <--- Use this instead of setShaderMVP
